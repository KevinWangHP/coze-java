...
public class AudioChatExample {
                              ...
                              try {
                                ...
                                CreateSpeechReq speechReq =
                                    CreateSpeechReq.builder()
                                        ...
                                        .voiceID(voiceID)
                                        .responseFormat(
                                            com.coze.openapi.client.audio.common.AudioFormat.WAV)
                                        .sampleRate(24000)
                                        .build();
  ...
  private static void createConversation(CozeAPI coze, String botID, String userID)
      throws Exception {
      ...
      if (bytesRead > 0) {
        ...
        if (System.currentTimeMillis() - lastTranscriptUpdateTime > TRANSCRIPTION_TIMEOUT) {
          ...
          if (!transcriptionSegments.isEmpty()) {
            ...
            // 调用bot处理
            callBotAndProcessResponse(
                coze, botID, "default_user_id", lastTranscription, voiceID, transcriptionClient);
            // 清空转录片段
