...
public class PerformanceLogger {
  ...
  public void logAsrResult(String recognizedText) {
    ...
    System.out.println("[性能日志] ASR - 音频时长: " + audioDurationMs + "ms, 识别时延: " + recognitionLatencyMs + "ms, 总会话: " + totalSessionMs + "ms, 文本长度: " + textLength);
  ...
  public void logWorkflowResult(String inputText, String outputText) {
    ...
    System.out.println("[性能日志] Workflow - 输入长度: " + inputLength + ", 输出长度: " + outputLength + ", 时延: " + latencyMs + "ms");
  }
  ...
  public void logTtsResult(String text, byte[] audioData, int audioSampleRate) {
    ...
    // 时长(秒) = 字节数 / (采样率 * 每样本字节数)
    double audioDurationSec = audioSampleRate > 0 ?
        (double) audioBytes / (audioSampleRate * 2) : 0;
    ...
    System.out.println("[性能日志] TTS - 输入长度: " + textLength + ", 输出大小: " + audioBytes +
        " bytes, 音频时长: " + String.format("%.2f", audioDurationSec) + "s, 合成时延: " + latencyMs + "ms");
  }

  /**
   * 记录完整的端到端流程统计
   * 端到端时延 = 从ASR最后一次收到音频到TTS开始播放的时间
   */
